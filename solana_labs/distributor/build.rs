...
fn main() -> Result<()> {
    // load IDL
    let path = "idls/distributor.json";
    ...
    for ix in &idl.instructions {
        ...
        let args_ty = format_ident!("{}IxData", pascal);  // Renamed to avoid collision with typedefs
        ...
        let args_fields = ix.args.iter().map(|arg| {
            ...
            let field_tokens = match &arg.ty {
                ...
                // Defined types need to be qualified with typedefs:: to avoid self-reference
                IdlType::Defined { name, generics: _ } => {
                    let ident = format_ident!("{}", name);
                    quote! {
                        pub #field_ident: typedefs::#ident,
                    }
                }
                _ => {
                    quote! {
    ...
    let variants = idl.instructions.iter().map(|ix| {
        ...
        let var = format_ident!("{}", pascal);
        let args_ty = format_ident!("{}IxData", pascal);  // Match the renamed type
        let accounts_ty = format_ident!("{}Accounts", pascal);

...
pub mod accounts_data {
    ...
    #[derive(Debug, Serialize)]
    pub struct CollectRewardsAccounts {
        pub user: String,
        pub strategy: String,
        pub globalConfig: String,
        pub baseVaultAuthority: String,
...
pub mod ix_data {
    ...
    #[derive(:: borsh :: BorshDeserialize, Debug, Serialize)]
    pub struct CollectRewardsArguments {}
    #[derive(:: borsh :: BorshDeserialize, Debug, Serialize)]
    pub struct CollectFeesArguments {}
    #[derive(:: borsh :: BorshDeserialize, Debug, Serialize)]
...
pub enum Instruction {
    ...
    CollectRewards {
        ...
        args: CollectRewardsArguments,
    },
    CollectFees {
        accounts: CollectFeesAccounts,
        args: CollectFeesArguments,
    },
    SwapRewards {
...
impl Instruction {
    pub fn decode(account_keys: &[String], data: &[u8]) -> anyhow::Result<Self> {
        ...
        match disc {
            ...
            [63u8, 130u8, 90u8, 197u8, 39u8, 16u8, 143u8, 176u8] => {
                let mut rdr: &[u8] = rest;
                let args = CollectRewardsArguments::deserialize(&mut rdr)?;
                let mut keys = account_keys.iter();
                let user = keys.next().unwrap().clone();
                let strategy = keys.next().unwrap().clone();
                let globalConfig = keys.next().unwrap().clone();
                ...
                let eventAuthority = keys.next().unwrap().clone();
                let remaining = keys.cloned().collect();
                let accounts = CollectRewardsAccounts {
                    user,
                    strategy,
                    globalConfig,
                    baseVaultAuthority,
                ...
                return Ok(Instruction::CollectRewards { accounts, args });
            }
            [164u8, 152u8, 207u8, 99u8, 30u8, 186u8, 19u8, 182u8] => {
                let mut rdr: &[u8] = rest;
                let args = CollectFeesArguments::deserialize(&mut rdr)?;
                let mut keys = account_keys.iter();
                let user = keys.next().unwrap().clone();
                let strategy = keys.next().unwrap().clone();
                let globalConfig = keys.next().unwrap().clone();
                ...
                return Ok(Instruction::CollectFees { accounts, args });
            }

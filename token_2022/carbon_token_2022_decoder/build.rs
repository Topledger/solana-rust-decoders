...
fn main() -> Result<()> {
    ...
    let path = "idls/carbon_token_2022_decoder.json";
    let raw = fs::read_to_string(path)?;
    
    // Normalize the IDL format to match anchor expectations
    let mut normalized = raw.replace(r#"\"writable\""#, r#"\"mut\""#);
    
    // Convert discriminant format to discriminator format
    // Replace "discriminant": {"type": "u8", "value": X} with "discriminator": [X, 0, 0, 0, 0, 0, 0, 0]
    // and handle bytes format as well
    let discriminant_regex = Regex::new(r#""discriminant":\s*\{\s*"type":\s*"u8",\s*"value":\s*(\d+)\s*\}"#)?;
    normalized = discriminant_regex.replace_all(&normalized, |caps: &Captures| {
        let value: u8 = caps[1].parse().unwrap_or(0);
        format!(r#""discriminator": [{}, 0, 0, 0, 0, 0, 0, 0]"#, value)
    }).to_string();
    
    // Handle bytes discriminant format
    let bytes_discriminant_regex = Regex::new(r#""discriminant":\s*\{\s*"type":\s*"bytes",\s*"value":\s*\[([^\]]+)\]\s*\}"#)?;
    normalized = bytes_discriminant_regex.replace_all(&normalized, |caps: &Captures| {

//! Refactored instruction decoder with no Substreams dependency.
//! Input accounts are passed directly as arrays of pubkey strings.
...
use crate::pb::sf::solana::orca_whirlpool::v1::{
    ...
    PbClosePositionWithTokenExtensionsLayout,
    PbPubKey,
...
pub const INSTRUCTION_TYPES: [(&[u8], &str); 50] = [
    ...
    (&[186, 143, 209, 29, 254, 2, 194, 117], "TwoHopSwapV2"),
    (&[1, 182, 135, 59, 155, 25, 99, 223], "ClosePositionWithTokenExtensions"),
];

pub fn process_instruction(data: &[u8], accounts: &[String]) -> anyhow::Result<Meta> {
    // --- Start of original logic ---

    let (inst_type, decoded_args) = decode_args(&data)
            .map_err(|e| anyhow::anyhow!("decode_args failed: {}", e))?;
    ...
    match decoded_args {
        ...
        EnumInstructionArgs::SwapV2(swap) => {
            // Separate V2 handling
            flat_args.amount = swap.amount.map(|v| v.to_string());
            flat_args.other_amount_threshold = swap.other_amount_threshold.map(|v| v.to_string());
            flat_args.sqrt_price_limit = swap.sqrt_price_limit.and_then(|p| p.value); // Direct access
            flat_args.amount_specified_is_input = swap.amount_specified_is_input;
        ...
        EnumInstructionArgs::TwoHopSwap(two_hop) => {
            ...
            flat_args.other_amount_threshold =
                two_hop.other_amount_threshold.map(|v| v.to_string());
            ...
            flat_args.sqrt_price_limit_two = two_hop.sqrt_price_limit_two.and_then(|p| p.value);
            // Direct access
        }
        EnumInstructionArgs::TwoHopSwapV2(two_hop) => {
            // Separate V2 handling
            flat_args.amount = two_hop.amount.map(|v| v.to_string());
            flat_args.other_amount_threshold =
                two_hop.other_amount_threshold.map(|v| v.to_string());
        ...
        EnumInstructionArgs::InitializePool(pool) => {
            ...
            flat_args.sqrt_price_limit = pool.initial_sqrt_price.and_then(|p| p.value);
            // Direct access
        }
        EnumInstructionArgs::InitializePoolV2(pool) => {
            // Separate V2 handling
            flat_args.tick_spacing = pool.tick_spacing;
            flat_args.sqrt_price_limit = pool.initial_sqrt_price.and_then(|p| p.value);
            // Direct access
        ...
        EnumInstructionArgs::SetRewardEmissions(reward) => {
            ...
            flat_args.emissions_per_second_x64 =
                reward.emissions_per_second_x64.and_then(|p| p.value); // Direct access
        }
        EnumInstructionArgs::SetRewardEmissionsV2(reward) => {
            // Separate V2 handling
            flat_args.reward_index = reward.reward_index;
            flat_args.emissions_per_second_x64 =
                reward.emissions_per_second_x64.and_then(|p| p.value); // Direct access
        ...
        EnumInstructionArgs::IncreaseLiquidityV2(liq) => {
            // Separate V2 handling
            flat_args.liquidity_amount = liq.liquidity_amount.and_then(|p| p.value); // Direct access
            flat_args.token_max_a = liq.token_max_a.map(|v| v.to_string());
            flat_args.token_max_b = liq.token_max_b.map(|v| v.to_string());
        ...
        EnumInstructionArgs::DecreaseLiquidityV2(liq) => {
            // Separate V2 handling
            flat_args.liquidity_amount = liq.liquidity_amount.and_then(|p| p.value); // Direct access
            flat_args.token_min_a = liq.token_min_a.map(|v| v.to_string());
            flat_args.token_min_b = liq.token_min_b.map(|v| v.to_string());
        ...
        EnumInstructionArgs::CollectRewardV2(reward) => {
            // Separate V2 handling
            flat_args.reward_index = reward.reward_index;
        }
    ...
    // Wrap result in Some()
    let input_accounts = prepare_input_accounts::prepare_input_accounts(inst_type.clone(), &accounts);
...
/// Compute the 8-byte discriminator for a given instruction name.
fn compute_discriminator(instruction_name: &str) -> [u8; 8] {
    // Format and hash logic here...
...
/// Decode raw instruction bytes into (inst_type, args enum).
pub fn decode_args(data: &[u8]) -> Result<(String, EnumInstructionArgs), String> {
    if data.len() < 8 {
    ...
    let inst_type = match INSTRUCTION_TYPES
        .iter()
...
pub fn decode_instruction_args(
    inst_type: &str,
    data: &[u8],
) -> Result<EnumInstructionArgs, String> {
    ...
    fn bytes_to_pb_u128(bytes: &[u8]) -> Result<Option<PbUint128>, String> {
        ...
        let val = u128::from_le_bytes(
            bytes
                .try_into()
                .map_err(|_| "Failed to convert slice to u128 array")?,
        );
        Ok(Some(PbUint128 {
    ...
    macro_rules! check_len {
        ($data:expr, $expected:expr, $name:expr) => {
            if $data.len() < $expected {
                return Err(format!(
                    "{}: Insufficient data length. Expected {}, got {}",
                    $name,
                    $expected,
                    $data.len()
                ));
    ...
    let args = match inst_type {
        "Swap" => {
            ...
            let amount = u64::from_le_bytes(
                data[0..8]
                    .try_into()
                    .map_err(|e| format!("Swap amount: {}", e))?,
            );
            let other_amount_threshold = u64::from_le_bytes(
                data[8..16]
                    .try_into()
                    .map_err(|e| format!("Swap threshold: {}", e))?,
            );
        ...
        "InitializePool" => {
            ...
            let bumps = PbBumps {
                whirlpool_bump: Some(data[0] as u32),
            };
            let tick_spacing = u16::from_le_bytes(
                data[1..3]
                    .try_into()
                    .map_err(|e| format!("InitializePool tick_spacing: {}", e))?,
            );
            let initial_sqrt_price = bytes_to_pb_u128(&data[3..19])?;
            Ok(arg::InstructionArgs::InitializePool(
                PbInitializePoolLayout {
                    bumps: Some(bumps),
        ...
        "SetRewardEmissions" => {
            ...
            Ok(arg::InstructionArgs::SetRewardEmissions(
                PbSetRewardEmissionsLayout {
        ...
        "IncreaseLiquidity" => {
            ...
            let token_max_a = u64::from_le_bytes(
                data[16..24]
                    .try_into()
                    .map_err(|e| format!("IncreaseLiquidity token_max_a: {}", e))?,
            );
            let token_max_b = u64::from_le_bytes(
                data[24..32]
                    .try_into()
                    .map_err(|e| format!("IncreaseLiquidity token_max_b: {}", e))?,
            );
            Ok(arg::InstructionArgs::IncreaseLiquidity(
                PbIncreaseLiquidityLayout {
                    liquidity_amount,
        ...
        "DecreaseLiquidity" => {
            ...
            let liquidity_amount = bytes_to_pb_u128(&data[0..16])?;
            let token_min_a = u64::from_le_bytes(
                data[16..24]
                    .try_into()
                    .map_err(|e| format!("DecreaseLiquidity token_min_a: {}", e))?,
            );
            let token_min_b = u64::from_le_bytes(
                data[24..32]
                    .try_into()
                    .map_err(|e| format!("DecreaseLiquidity token_min_b: {}", e))?,
            );
            Ok(arg::InstructionArgs::DecreaseLiquidity(
                PbDecreaseLiquidityLayout {
                    liquidity_amount,
        ...
        "TwoHopSwap" => {
            ...
            let amount = u64::from_le_bytes(
                data[0..8]
                    .try_into()
                    .map_err(|e| format!("TwoHopSwap amount: {}", e))?,
            );
            let other_amount_threshold = u64::from_le_bytes(
                data[8..16]
                    .try_into()
                    .map_err(|e| format!("TwoHopSwap threshold: {}", e))?,
            );
        ...
        "SwapV2" => {
            ...
            let amount = u64::from_le_bytes(
                data[0..8]
                    .try_into()
                    .map_err(|e| format!("SwapV2 amount: {}", e))?,
            );
            let other_amount_threshold = u64::from_le_bytes(
                data[8..16]
                    .try_into()
                    .map_err(|e| format!("SwapV2 threshold: {}", e))?,
            );
            let sqrt_price_limit = bytes_to_pb_u128(&data[16..32])?;
            let amount_specified_is_input = data[32] != 0;
            let a_to_b = data[33] != 0;
        ...
        "TwoHopSwapV2" => {
            ...
            let amount = u64::from_le_bytes(
                data[0..8]
                    .try_into()
                    .map_err(|e| format!("TwoHopSwapV2 amount: {}", e))?,
            );
            let other_amount_threshold = u64::from_le_bytes(
                data[8..16]
                    .try_into()
                    .map_err(|e| format!("TwoHopSwapV2 threshold: {}", e))?,
            );
            let amount_specified_is_input = data[16] != 0;
            let a_to_b_one = data[17] != 0;
            let a_to_b_two = data[18] != 0;
        ...
        "InitializeRewardV2" => {
            ...
            let reward_index = data[0];
            // remaining_accounts_info not decoded here
            Ok(arg::InstructionArgs::InitializeRewardV2(
                PbInitializeRewardV2Layout {
                    reward_index: Some(reward_index as u32),
                },
            ))
        ...
        "SetRewardEmissionsV2" => {
            ...
            let emissions_per_second_x64 = bytes_to_pb_u128(&data[1..17])?;
            // remaining_accounts_info not decoded here
            Ok(arg::InstructionArgs::SetRewardEmissionsV2(
                PbSetRewardEmissionsV2Layout {
                    reward_index: Some(reward_index as u32),
                    emissions_per_second_x64,
                },
        ...
        "IncreaseLiquidityV2" => {
            ...
            let token_max_a = u64::from_le_bytes(
                data[16..24]
                    .try_into()
                    .map_err(|e| format!("IncreaseLiquidityV2 token_max_a: {}", e))?,
            );
            let token_max_b = u64::from_le_bytes(
                data[24..32]
                    .try_into()
                    .map_err(|e| format!("IncreaseLiquidityV2 token_max_b: {}", e))?,
            );
            // remaining_accounts_info not decoded here
            Ok(arg::InstructionArgs::IncreaseLiquidityV2(
                PbIncreaseLiquidityV2Layout {
                    liquidity_amount,
                    token_max_a: Some(token_max_a),
                    token_max_b: Some(token_max_b),
        ...
        "DecreaseLiquidityV2" => {
            ...
            let token_min_a = u64::from_le_bytes(
                data[16..24]
                    .try_into()
                    .map_err(|e| format!("DecreaseLiquidityV2 token_min_a: {}", e))?,
            );
            let token_min_b = u64::from_le_bytes(
                data[24..32]
                    .try_into()
                    .map_err(|e| format!("DecreaseLiquidityV2 token_min_b: {}", e))?,
            );
            // remaining_accounts_info not decoded here
            Ok(arg::InstructionArgs::DecreaseLiquidityV2(
                PbDecreaseLiquidityV2Layout {
                    liquidity_amount,
                    token_min_a: Some(token_min_a),
                    token_min_b: Some(token_min_b),
        ...
        "CollectRewardV2" => {
            ...
            let reward_index = data[0];
            // remaining_accounts_info not decoded here
            Ok(arg::InstructionArgs::CollectRewardV2(
                PbCollectRewardV2Layout {
                    reward_index: Some(reward_index as u32),
                    remaining_accounts_info: None, // Set if defined
                },
        ...
        "InitializeConfig" => {
            ...
            let default_protocol_fee_rate = u16::from_le_bytes(
                data[96..98]
                    .try_into()
                    .map_err(|e| format!("InitializeConfig fee_rate: {}", e))?,
            );
            Ok(arg::InstructionArgs::InitializeConfig(
                PbInitializeConfigLayout {
                    fee_authority,
        ...
        "InitializeTickArray" => {
            ...
            let start_tick_index = i32::from_le_bytes(
                data[0..4]
                    .try_into()
                    .map_err(|e| format!("InitializeTickArray index: {}", e))?,
            );
            Ok(arg::InstructionArgs::InitializeTickArray(
                PbInitializeTickArrayLayout {
        ...
        "InitializeFeeTier" => {
            ...
            let tick_spacing = u16::from_le_bytes(
                data[0..2]
                    .try_into()
                    .map_err(|e| format!("InitializeFeeTier spacing: {}", e))?,
            );
            let default_fee_rate = u16::from_le_bytes(
                data[2..4]
                    .try_into()
                    .map_err(|e| format!("InitializeFeeTier rate: {}", e))?,
            );
            Ok(arg::InstructionArgs::InitializeFeeTier(
                PbInitializeFeeTierLayout {
                    tick_spacing: Some(tick_spacing as u32),
        ...
        "InitializeReward" => {
            // Shares layout with CollectReward
            check_len!(data, 1, "InitializeReward");
            let reward_index = data[0];
            Ok(arg::InstructionArgs::InitializeReward(
                PbInitializeRewardLayout {
                    reward_index: Some(reward_index as u32),
                },
            ))
        ...
        "OpenPositionWithMetadata" => {
            ...
            let tick_lower_index = i32::from_le_bytes(
                data[2..6]
                    .try_into()
                    .map_err(|e| format!("OpenPositionWithMetadata lower: {}", e))?,
            );
            let tick_upper_index = i32::from_le_bytes(
                data[6..10]
                    .try_into()
                    .map_err(|e| format!("OpenPositionWithMetadata upper: {}", e))?,
            );
            Ok(arg::InstructionArgs::OpenPositionWithMetadata(
                PbOpenPositionWithMetadataLayout {
        ...
        "OpenPositionWithTokenExtensions" => {
            ...
            let tick_lower_index = i32::from_le_bytes(
                data[0..4]
                    .try_into()
                    .map_err(|e| format!("OpenPositionWithTokenExtensions lower: {}", e))?,
            );
            let tick_upper_index = i32::from_le_bytes(
                data[4..8]
                    .try_into()
                    .map_err(|e| format!("OpenPositionWithTokenExtensions upper: {}", e))?,
            );
        ...
        "SetDefaultFeeRate" => {
            ...
            let default_fee_rate = u16::from_le_bytes(
                data[0..2]
                    .try_into()
                    .map_err(|e| format!("SetDefaultFeeRate rate: {}", e))?,
            );
            Ok(arg::InstructionArgs::SetDefaultFeeRate(
                PbSetDefaultFeeRateLayout {
                    default_fee_rate: Some(default_fee_rate as u32),
                },
            ))
        ...
        "SetDefaultProtocolFeeRate" => {
            ...
            let default_protocol_fee_rate = u16::from_le_bytes(
                data[0..2]
                    .try_into()
                    .map_err(|e| format!("SetDefaultProtocolFeeRate rate: {}", e))?,
            );
            Ok(arg::InstructionArgs::SetDefaultProtocolFeeRate(
                PbSetDefaultProtocolFeeRateLayout {
                    default_protocol_fee_rate: Some(default_protocol_fee_rate as u32),
                },
            ))
        ...
        "SetFeeRate" => {
            ...
            let fee_rate = u16::from_le_bytes(
                data[0..2]
                    .try_into()
                    .map_err(|e| format!("SetFeeRate rate: {}", e))?,
            );
        ...
        "SetProtocolFeeRate" => {
            ...
            let protocol_fee_rate = u16::from_le_bytes(
                data[0..2]
                    .try_into()
                    .map_err(|e| format!("SetProtocolFeeRate rate: {}", e))?,
            );
            Ok(arg::InstructionArgs::SetProtocolFeeRate(
                PbSetProtocolFeeRateLayout {
        ...
        "SetRewardAuthority" => {
            ...
            let reward_index = data[0];
            Ok(arg::InstructionArgs::SetRewardAuthority(
                PbSetRewardAuthorityLayout {
                    reward_index: Some(reward_index as u32),
                },
            ))
        ...
        "SetRewardAuthorityBySuperAuthority" => {
            ...
            let reward_index = data[0];
            Ok(arg::InstructionArgs::SetRewardAuthorityBySuperAuthority(
                PbSetRewardAuthorityBySuperAuthorityLayout {
                    reward_index: Some(reward_index as u32),
                },
            ))
        ...
        "OpenBundledPosition" => {
            ...
            let bundle_index = u16::from_le_bytes(
                data[0..2]
                    .try_into()
                    .map_err(|e| format!("OpenBundledPosition index: {}", e))?,
            );
            let tick_lower_index = i32::from_le_bytes(
                data[2..6]
                    .try_into()
                    .map_err(|e| format!("OpenBundledPosition lower: {}", e))?,
            );
            let tick_upper_index = i32::from_le_bytes(
                data[6..10]
                    .try_into()
                    .map_err(|e| format!("OpenBundledPosition upper: {}", e))?,
            );
            Ok(arg::InstructionArgs::OpenBundledPosition(
                PbOpenBundledPositionLayout {
        ...
        "CloseBundledPosition" => {
            ...
            let bundle_index = u16::from_le_bytes(
                data[0..2]
                    .try_into()
                    .map_err(|e| format!("CloseBundledPosition index: {}", e))?,
            );
            Ok(arg::InstructionArgs::CloseBundledPosition(
                PbCloseBundledPositionLayout {
                    bundle_index: Some(bundle_index as u32),
                },
            ))
        ...
        "UpdateFeesAndRewards" => Ok(arg::InstructionArgs::UpdateFeesAndRewards(
            PbUpdateFeesAndRewardsLayout::default(),
        )),
        "CollectFees" => Ok(arg::InstructionArgs::CollectFees(
            PbCollectFeesLayout::default(),
        )),
        "CollectProtocolFees" => Ok(arg::InstructionArgs::CollectProtocolFees(
            PbCollectProtocolFeesLayout::default(),
        )),
        "ClosePosition" => Ok(arg::InstructionArgs::ClosePosition(
            PbClosePositionLayout::default(),
        )),
        "SetFeeAuthority" => Ok(arg::InstructionArgs::SetFeeAuthority(
            PbSetFeeAuthorityLayout::default(),
        )),
        "SetCollectProtocolFeesAuthority" => {
            Ok(arg::InstructionArgs::SetCollectProtocolFeesAuthority(
                PbSetCollectProtocolFeesAuthorityLayout::default(),
            ))
        }
        "SetRewardEmissionsSuperAuthority" => {
            Ok(arg::InstructionArgs::SetRewardEmissionsSuperAuthority(
                PbSetRewardEmissionsSuperAuthorityLayout::default(),
            ))
        }
        "InitializePositionBundle" => Ok(arg::InstructionArgs::InitializePositionBundle(
            PbInitializePositionBundleLayout::default(),
        )),
        "InitializePositionBundleWithMetadata" => {
            Ok(arg::InstructionArgs::InitializePositionBundleWithMetadata(
                PbInitializePositionBundleWithMetadataLayout::default(),
            ))
        }
        "DeletePositionBundle" => Ok(arg::InstructionArgs::DeletePositionBundle(
            PbDeletePositionBundleLayout::default(),
        )),
        "InitializeConfigExtension" => Ok(arg::InstructionArgs::InitializeConfigExtension(
            PbInitializeConfigExtensionLayout::default(),
        )),
        "SetConfigExtensionAuthority" => Ok(arg::InstructionArgs::SetConfigExtensionAuthority(
            PbSetConfigExtensionAuthorityLayout::default(),
        )),
        "SetTokenBadgeAuthority" => Ok(arg::InstructionArgs::SetTokenBadgeAuthority(
            PbSetTokenBadgeAuthorityLayout::default(),
        )),
        "InitializeTokenBadge" => Ok(arg::InstructionArgs::InitializeTokenBadge(
            PbInitializeTokenBadgeLayout::default(),
        )),
        "DeleteTokenBadge" => Ok(arg::InstructionArgs::DeleteTokenBadge(
            PbDeleteTokenBadgeLayout::default(),
        )),
        "InitializeAccount" => Ok(arg::InstructionArgs::InitializeAccount(
            PbInitializeAccountLayout::default(),
        )),
        "IdlWrite" => Ok(arg::InstructionArgs::IdlWrite(PbIdlWriteLayout::default())), // Args not decoded
        "CollectFeesV2" => Ok(arg::InstructionArgs::CollectFeesV2(
            PbCollectFeesV2Layout::default(),
        )), // Args not decoded
        "CollectProtocolFeesV2" => Ok(arg::InstructionArgs::CollectProtocolFeesV2(
            PbCollectProtocolFeesV2Layout::default(),
        )), 
        "ClosePositionWithTokenExtensions" => {
          
            
            // bumps not in this layout?
            Ok(arg::InstructionArgs::ClosePositionWithTokenExtensions(
                PbClosePositionWithTokenExtensionsLayout {
                },
            ))
        }// Args not decoded
        ...
        _ => Err(format!(
            "Unknown or unhandled instruction type for manual decoding: {}",
            inst_type
        )),
    };
